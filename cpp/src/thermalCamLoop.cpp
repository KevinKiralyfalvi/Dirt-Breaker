...
#include "thermalCamLoop.hpp"
#include "capture.hpp"
#include <mutex>
#include <opencv2/imgproc.hpp>

void thermalCamLoop(cv::VideoCapture &gunCamera, int bufferID)
{
    std::mutex frameMutex;
    cv::Mat rawFrame;
    cv::Mat convertedResizedFrame;
    ...
    while (true)
    {
        ...
        cv::resize(convertedFrame, convertedResizedFrame, cv::Size(), 2, 2, cv::INTER_NEAREST);
        memcpy(buffer, convertedResizedFrame.data, convertedResizedFrame.total() * sizeof(uint16_t));
    }
}
